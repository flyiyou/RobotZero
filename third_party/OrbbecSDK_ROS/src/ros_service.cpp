...
void OBCameraNode::setupCameraCtrlServices() {
  ...
  get_flood_status_srv_ = nh_.advertiseService<GetBoolRequest, GetBoolResponse>(
    "/" + camera_name_ + "/" + "get_floor_status",
    [this](GetBoolRequest& request, GetBoolResponse& response) {
      response.success = this->getFloodStatusCallback(request, response);
      return response.success;
    });
  get_flood_level_srv_ = nh_.advertiseService<GetInt32Request, GetInt32Response>(
    "/" + camera_name_ + "/" + "get_floor_level",
    [this](GetInt32Request& request, GetInt32Response& response) {
      response.success = this->getFloodLevelCallback(request, response);
      return response.success;
    });
  set_flood_level_srv_ = nh_.advertiseService<SetInt32Request, SetInt32Response>(
    "/" + camera_name_ + "/" + "set_floor_level",
    [this](SetInt32Request& request, SetInt32Response& response) {
      response.success = this->setFloodLevelCallback(request, response);
      return response.success;
    });
  get_ir_channel_srv_ = nh_.advertiseService<GetInt32Request, GetInt32Response>(
    "/" + camera_name_ + "/" + "get_ir_channel",
    [this](GetInt32Request& request, GetInt32Response& response) {
      response.success = this->getIRSwitchStatusCallback(request, response);
      return response.success;
    });
...
bool OBCameraNode::getFloodStatusCallback(GetBoolRequest& request, GetBoolResponse& response) {
  (void)request;
  std::lock_guard<decltype(device_lock_)> lock(device_lock_);
  try {
    response.data = device_->getBoolProperty(OB_PROP_FLOOD_BOOL);
  } catch (const ob::Error& e) {
    ROS_ERROR_STREAM("Failed to get LDP status: " << e.getMessage());
    response.success = false;
    return false;
...
bool OBCameraNode::getFloodLevelCallback(GetInt32Request& request, GetInt32Response& response){
  (void)request;
  std::lock_guard<decltype(device_lock_)> lock(device_lock_);
  try {
    response.data = device_->getIntProperty(OB_PROP_FLOOD_LEVEL_INT);
  } catch (const ob::Error& e) {
    ROS_ERROR_STREAM("Failed to get LDP status: " << e.getMessage());
    response.success = false;
    return false;
...
bool OBCameraNode::setFloodLevelCallback(SetInt32Request& request, SetInt32Response& response){
  (void)response;
  std::lock_guard<decltype(device_lock_)> lock(device_lock_);
  try {
    device_->setIntProperty(OB_PROP_FLOOD_LEVEL_INT, request.data);
  } catch (const ob::Error& e) {
    ROS_ERROR_STREAM("Failed to get LDP status: " << e.getMessage());
    response.success = false;
    return false;
...
bool OBCameraNode::getIRSwitchStatusCallback(GetInt32Request& request, GetInt32Response& response){
  (void)request;
  std::lock_guard<decltype(device_lock_)> lock(device_lock_);
  try {
    response.data = device_->getIntProperty(OB_PROP_FLOOD_LEVEL_INT);
